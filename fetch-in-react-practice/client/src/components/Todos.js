...
export default function Todos() {
  ...
  async function toggleCompleted(todoId) {
    const selectedTodo = todos.find((todo) => todo.todoId === todoId );
    fetch(`/api/todos/${todoId}`, {
     method: 'PATCH',
     headers: { 'Content-Type': 'application/json' },
     body: JSON.stringify({isCompleted: !selectedTodo.isCompleted})
     })
      .then(res => {
       if (!res.ok) {
           throw new Error(`status code ${res.status}`);
      ...
      .then(res => {
        const updateTodo = todos.map((currentTodo) => currentTodo.todoId === res.todoId ? res : currentTodo);
        setTodos(updateTodo);
      })
       .catch(err => {
         setError(err);
 ...
 *
 *  DO NOT MUTAT the original state array, nor any objects within it.
 *
 ...
 *       of the old array, plus the object returned by the server.E
 */
